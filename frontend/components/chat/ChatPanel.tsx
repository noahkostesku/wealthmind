...
import { Send, BotMessageSquare, X, Check, Bell, ChevronRight } from "lucide-react";
import { createChatSession, clearChatSession, streamChatMessage, dismissMonitorAlert } from "@/lib/api";
import type { ChatSession, MonitorAlertData } from "@/types";
import { AGENT_CAPABILITIES, DEFAULT_CHIPS } from "@/components/onboarding/WellyIntro";
...
function formatContent(text: string) {
  ...
  // Split on dollar amounts, markdown links [Title](url), and newlines
  const parts = clean.split(/(\$[\d,]+(?:\.\d{2})?|\[[^\]]+\]\([^)]+\)|\n)/g);
...
interface ChatPanelProps {
  onClose?: () => void;
  /** Collapses to thin strip (desktop); different from onClose */
  onCollapse?: () => void;
  /** Pre-fill the input from an external source (WellyCallout) */
  externalInput?: string;
  /** Called after externalInput is consumed so AppShell can clear it */
  ...
  /** Called when the bell icon is clicked — parent should expand panel + clear badge */
  onBellClick?: () => void;
}
...
export function ChatPanel({
  ...
  // Consume external input (from WellyCallout "Ask Welly" button)
  useEffect(() => {
  ...
  const send = useCallback(
    async (text: string) => {
      ...
      if (text.trim().toLowerCase() === "welly-clear") {
        ...
        // Replay the same intro sequence shown on first login
        const t = Date.now();
        setMessages([{
          id: `w1-${t}`,
          role: "assistant",
          content: "I'm Welly. I can see all your accounts — your TFSA, RRSP, chequing, non-registered portfolio, crypto, and margin. I work with five specialist agents to find opportunities in your finances.",
        }]);
  ...
  function handleBellClick() {
    onBellClick?.();
    // Wait for panel expansion animation (200ms CSS transition) then scroll
    setTimeout(() => {
      const firstAlert = messagesContainerRef.current?.querySelector("[data-is-alert]");
      firstAlert?.scrollIntoView({ behavior: "smooth", block: "nearest" });
    }, 220);
  }
    ...
    <div className="flex flex-col h-full bg-white border-l border-[#E5E5E5]">
      ...
      <div className="h-14 px-4 flex items-center gap-2.5 border-b border-[#E5E5E5] flex-shrink-0">
        ...
        {onCollapse && (
          <button
            onClick={onCollapse}
            className="hidden lg:flex p-1.5 rounded-lg text-[#6B7280] hover:text-[#111827] hover:bg-zinc-100 transition-colors flex-shrink-0"
            title="Collapse Welly"
          >
        ...
        {unreadAlertCount != null && unreadAlertCount > 0 && (
          <button
            ...
            className="relative flex items-center p-1 rounded-lg hover:bg-amber-50 transition-colors"
            title="Jump to alerts"
          >
            <Bell className="w-4 h-4 text-amber-500" />
            <span className="absolute -top-0.5 -right-0.5 w-4 h-4 rounded-full bg-amber-400 text-white text-[9px] font-bold flex items-center justify-center leading-none">
      ...
      <div ref={messagesContainerRef} className="flex-1 overflow-y-auto p-4 flex flex-col gap-3">
        {initError ? (
          ...
          <>
            {messages.map((msg) => (
              <div key={msg.id} {...(msg.monitorAlert ? { "data-is-alert": "" } : {})}>
                <MessageBubble
                  msg={msg}
                  onChip={send}
